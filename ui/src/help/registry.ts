...
const TOPICS: Record<string, HelpTopic> = {
  ...
  cardholders: {
    ...
    icon: '🪪',
    sections: [
      { body: 'A cardholder is a person in the system — staff, contractor, or visitor. Their roles determine what they can open; their credentials are the cards/PINs they present.' },
      { heading: 'Permanent and visitors', body: 'Both are cardholders — a visitor is one with a pass that expires — so they share this page and the filter chooses between them. It opens on Permanent, so a lobby full of guests does not bury your roster, and remembers whichever you last chose. The same person can be either over time; a returning visitor is recognised rather than duplicated.' },
      { heading: 'Searching', body: 'Search always looks at everyone, whichever filter is showing. If a name matches someone the filter is hiding, the page says how many and offers to show them — so a guest is never invisible just because you were looking at the roster.' },
      { heading: 'Pass state', body: 'The pass-state chips appear under Visitors only. A state comes from a person’s newest credential, which lives in another collection, so it narrows the loaded page rather than the whole collection. That is sound for visitors, sorted newest-first — the recent page is where every live visit is — and would mislead on a roster of thousands sorted by name.' },
      ...
      { heading: 'Passwords', body: 'A badge holder normally signs in with a password, or an emailed one-time code. Setting an initial password is optional but is the only way in on an install with no mail server — hand it over in person, it is never emailed. They can change it themselves from the account menu on their badge page.' },
      ...
      { heading: 'Reissue, don’t extend', body: 'A visitor’s pass is reissued, not edited: Reissue gives them a new pass with a new window and revokes the old code. There is deliberately no “extend”, and no way to add a pass to a visitor by hand — a visitor’s QR is a working key on a screen, so it gets photographed and forwarded, and pushing its expiry out would silently re-arm every copy of it. A reissue costs the visitor one refresh of their badge.' },
      { heading: 'Revoke vs delete', body: 'Revoke ends a visit: the pass stops opening doors immediately, including the QR code, and the person stays on the record so a return visit refreshes them instead of creating a duplicate. Delete removes the person, their credentials, and the fact that they were here. Reach for revoke.' },
      ...
      { heading: 'The QR code', body: 'A visitor’s QR carries their actual credential value, so a scanner can read it — which is why the pass is short-lived. A permanent badge’s QR carries only an identifier that opens nothing, because a lanyard badge gets photographed for years.' },
    ],
  },
  visitors: {
    ...
    icon: '👋',
    sections: [
      { body: 'Issuing a visitor pass creates the person, a time-bound credential, and their sign-in in a single step, and emails them where to view their badge. Afterwards they are an ordinary cardholder with `kind = visitor` — found, viewed, and revoked on the Cardholders page under its Visitors filter.' },
      { heading: 'Why this is not the cardholder form', body: 'The pass value comes from the server, not the browser: it is a key to a building, and one supplied by a form could be guessable or made to collide. Person and credential are created in one transaction too, because a half-created visitor is worse than a clean failure.' },
      { heading: 'A returning visitor is reused', body: 'Someone who has visited before is matched by email and refreshed in place, with their previous code revoked — the same path the Reissue button takes. They are not duplicated, and their earlier visits stay on the record.' },
      { heading: 'No mail server', body: 'A visitor’s normal route in is an emailed one-time code, so with no mail configured they cannot sign in at all. Set an initial password here and hand it over at the desk — it is never emailed — or give them the badge link (and its QR) from the success screen. The link carries no code or token.' },
