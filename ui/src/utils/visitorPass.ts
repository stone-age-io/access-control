 ...
 * # Why it is shared
 *
 * The cardholder list (in its Visitors filter) and the cardholder detail page both answer
 * "is this pass working". They were separate derivations on two separate pages, which is how
 * the same visit ends up described two ways — the kind of discrepancy that teaches an
 * operator to stop trusting the console. One function, one answer. It lives in `utils`
 * rather than beside a view because both callers are now cardholder views and neither owns
 * it.
 *
 * # Ordering
 *
...
/** The state values the Visitors filter offers as chips, in the order they are shown. */
