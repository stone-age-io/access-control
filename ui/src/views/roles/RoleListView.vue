...
const { items: roles, totalItems, loading, error, load } = usePagination<Role>('roles', 50)
...
const columns: Column<Role>[] = [
  { key: 'code', label: 'Code' },
  { key: 'name', label: 'Name' },
  { key: 'access_groups', label: 'Groups', format: (v) => `${(v || []).length}` },
  { key: 'created', label: 'Created', format: (v) => formatDate(v, 'PP') },
]

function loadRoles() {
  load({ sort: 'code', expand: 'access_groups' })
}
...
<template>
  <div class="space-y-6">
    ...
    <BaseCard v-else :no-padding="true">
      <ResponsiveList :items="filtered" :columns="columns" :loading="loading" @row-click="(r) => router.push(`/roles/${r.id}/edit`)">
        ...
        <template #cell-access_groups="{ item }"><span class="badge badge-ghost badge-sm">{{ (item.access_groups || []).length }}</span></template>
        <template #card-access_groups="{ item }"><span class="badge badge-ghost badge-sm">{{ (item.access_groups || []).length }}</span></template>
      ...
      <div class="p-4 border-t border-base-300 text-sm text-base-content/60">{{ totalItems }} role(s)</div>
    </BaseCard>
  </div>
</template>
