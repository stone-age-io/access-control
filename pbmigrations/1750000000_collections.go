...
func init() {
	migrations.Register(func(app core.App) error {
		// --- sites: a building/campus; owns the timezone. ---
		sites := core.NewBaseCollection("sites")
		sites.Fields.Add(&core.TextField{Name: "code", Required: true})
		sites.Fields.Add(&core.TextField{Name: "name"})
		// timezone is an IANA name; validated in a hook (see internal/mirror).
		sites.Fields.Add(&core.TextField{Name: "timezone", Required: true})
		// fai_suppress: while the site's fire alarm input is active, suppress
		// forced/held-open alarms (hardware owns egress). Default true.
		sites.Fields.Add(&core.BoolField{Name: "fai_suppress"})
		addTimestamps(sites)
		sites.AddIndex("idx_sites_code", true, "code", "")
		...
		// --- access_points: a controllable opening (door/gate/turnstile/elevator). ---
		accessPoints := core.NewBaseCollection("access_points")
		accessPoints.Fields.Add(&core.TextField{Name: "code", Required: true})
		accessPoints.Fields.Add(&core.RelationField{
			Name:         "site",
			CollectionId: sites.Id,
			Required:     true,
			MaxSelect:    1,
		})
		accessPoints.Fields.Add(&core.TextField{Name: "name"})
		// posture: standing default; a runtime command may override it on the controller.
		accessPoints.Fields.Add(&core.SelectField{
			Name:      "posture",
			Values:    []string{"secure", "unlocked", "lockdown", "disabled"},
			MaxSelect: 1,
		})
		accessPoints.Fields.Add(&core.NumberField{Name: "pulse_seconds", OnlyInt: true})
		addTimestamps(accessPoints)
		accessPoints.AddIndex("idx_access_points_code", true, "code", "")
		if err := app.Save(accessPoints); err != nil {
			return err
		}

		// --- access_groups ("access levels"): a set of points under one schedule. ---
		...
		events.Fields.Add(&core.TextField{Name: "site"})
		events.Fields.Add(&core.TextField{Name: "access_point"})
		events.Fields.Add(&core.SelectField{
			Name:      "kind",
			Values:    []string{"tap", "state", "alarm", "fire", "command"},
			MaxSelect: 1,
		})
